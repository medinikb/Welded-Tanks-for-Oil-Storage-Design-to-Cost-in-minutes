...
## Small-tank Class 3 weight rule

TankM calculates the API 650 Core MTO first. For **small tanks only**:

- Small tank = **Core MTO < 100 MT OR diameter < 19 m**
- Predicted complete empty weight = **1.50 × Core MTO**
- Medium and large tank completion logic is unchanged.
- The historical global ridge prediction is retained as an audit diagnostic only; it does not set the final small-tank weight.

The factor is an owner-side Class 3 calibration relationship, not an API 650 equation and not a fabrication MTO rule.
