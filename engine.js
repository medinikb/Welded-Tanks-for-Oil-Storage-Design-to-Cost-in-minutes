(function(global){
  ...
  const VERSION="scope-governed-lstk";
  ...
  const SUPPLY_BILLING_IDIOT_INDEX=COST_DATA?(COST_DATA.FACTORS.supplyBilling??COST_DATA.FACTORS.materialSupply):2.1469839411750753;
  const BASE_LSTK_IDIOT_INDEX=COST_DATA?(COST_DATA.FACTORS.baseLSTK??COST_DATA.FACTORS.baseComplete):4.7710754248417135;
  const REFERENCE_LSTK_IDIOT_INDEX=COST_DATA?(COST_DATA.FACTORS.referenceLSTK??COST_DATA.FACTORS.installedReference):4.802926688446918;
  // Backward-compatible exported constant aliases.
  const POOLED_IDIOT_INDEX=SUPPLY_BILLING_IDIOT_INDEX;
  const BASE_COMPLETE_IDIOT_INDEX=BASE_LSTK_IDIOT_INDEX;
  const INSTALLED_REFERENCE_IDIOT_INDEX=REFERENCE_LSTK_IDIOT_INDEX;
  ...
  function estimateCost(input,mto,calibrated){
    ...
    if(!Number.isFinite(weightBasis)||weightBasis<=0){
      return {
        available:false,weightBasisMT:null,tankQuantity:qty,estimateYear:year,rawSteelRate:rawRate,
        rawSteelBasisINR:0,rawSteelBasisCr:0,
        supplyBillingCostINR:0,supplyBillingCostCr:0,
        nonSupplyLSTKBillingCostINR:0,nonSupplyLSTKBillingCostCr:0,
        baseLSTKCostINR:0,baseLSTKCostCr:0,
        directGroundReferenceCostINR:0,directGroundReferenceCostCr:0,
        referenceLSTKCostINR:0,referenceLSTKCostCr:0,
        selectedLSTKCostINR:0,selectedLSTKCostCr:0,
        weightRange:null,supplyBillingRange:null,nonSupplyLSTKRange:null,lstkRange:null,
        governance:"Cost withheld because a complete eligible weight estimate is unavailable."
      };
    }

    // New canonical override names, with fallback to legacy TankM input names.
    const supplyOverride=n(input.supplyBillingIdiotIndexOverride)>0?n(input.supplyBillingIdiotIndexOverride):(
      n(input.materialIdiotIndexOverride)>0?n(input.materialIdiotIndexOverride):(
        n(input.idiotIndexOverride)>0?n(input.idiotIndexOverride):SUPPLY_BILLING_IDIOT_INDEX
      )
    );
    const requestedBaseLSTKFactor=n(input.baseLSTKIdiotIndexOverride)>0?n(input.baseLSTKIdiotIndexOverride):(
      n(input.completeIdiotIndexOverride)>0?n(input.completeIdiotIndexOverride):BASE_LSTK_IDIOT_INDEX
    );
    const baseLSTKFactor=Math.max(supplyOverride,requestedBaseLSTKFactor);
    const requestedReferenceLSTKFactor=n(input.referenceLSTKIdiotIndexOverride)>0?n(input.referenceLSTKIdiotIndexOverride):(
      n(input.installedReferenceIdiotIndexOverride)>0?n(input.installedReferenceIdiotIndexOverride):REFERENCE_LSTK_IDIOT_INDEX
    );
    const referenceLSTKFactor=Math.max(baseLSTKFactor,requestedReferenceLSTKFactor);
    const nonSupplyLSTKBillingFactor=baseLSTKFactor-supplyOverride;
    const groundReferenceFactor=referenceLSTKFactor-baseLSTKFactor;

    const rawSteelBasis=(rawRate&&weightBasis>0)?weightBasis*1000*rawRate:0;
    const supplyBillingCost=rawSteelBasis*supplyOverride;
    const nonSupplyLSTKBillingCost=rawSteelBasis*nonSupplyLSTKBillingFactor;
    const baseLSTKCost=rawSteelBasis*baseLSTKFactor;
    const directGroundReferenceCost=rawSteelBasis*groundReferenceFactor;
    const referenceLSTKCost=rawSteelBasis*referenceLSTKFactor;
    const includeGroundReference=!!input.includeGroundReference;
    const selectedLSTKCost=includeGroundReference?referenceLSTKCost:baseLSTKCost;

    // Optional independent bottom-up cross-check. This is not added to the LSTK factor result.
    ...
    const supplyBillingRange=class3Range(supplyBillingCost,weightRange.lowPct,weightRange.highPct);
    const nonSupplyLSTKRange=class3Range(nonSupplyLSTKBillingCost,weightRange.lowPct,weightRange.highPct);
    const baseLSTKRange=class3Range(baseLSTKCost,weightRange.lowPct,weightRange.highPct);
    const referenceLSTKRange=class3Range(referenceLSTKCost,weightRange.lowPct,weightRange.highPct);
    const lstkRange=includeGroundReference?referenceLSTKRange:baseLSTKRange;

    const supplyBillingShareOfBasePct=baseLSTKCost>0?100*supplyBillingCost/baseLSTKCost:null;
    const nonSupplyShareOfBasePct=baseLSTKCost>0?100*nonSupplyLSTKBillingCost/baseLSTKCost:null;
    const directGroundReferenceSharePct=selectedLSTKCost>0?100*directGroundReferenceCost/selectedLSTKCost:null;

    const governance=(COST_DATA&&COST_DATA.AUDIT)?COST_DATA.AUDIT.governance:
      "Owner commercial screening factors; historical factors represent broad LSTK installed-package scope, not pure fabrication cost.";

    return {
      available:true,
      weightBasisMT:weightBasis,tankQuantity:qty,estimateYear:year,rawSteelRate:rawRate,
      rawSteelBasisINR:rawSteelBasis,rawSteelBasisCr:rawSteelBasis/1e7,

      supplyBillingIdiotIndex:supplyOverride,
      nonSupplyLSTKBillingIdiotIndex:nonSupplyLSTKBillingFactor,
      baseLSTKIdiotIndex:baseLSTKFactor,
      directGroundReferenceIdiotIndex:groundReferenceFactor,
      referenceLSTKIdiotIndex:referenceLSTKFactor,

      supplyBillingCostINR:supplyBillingCost,supplyBillingCostCr:supplyBillingCost/1e7,
      nonSupplyLSTKBillingCostINR:nonSupplyLSTKBillingCost,nonSupplyLSTKBillingCostCr:nonSupplyLSTKBillingCost/1e7,
      baseLSTKCostINR:baseLSTKCost,baseLSTKCostCr:baseLSTKCost/1e7,
      directGroundReferenceCostINR:directGroundReferenceCost,directGroundReferenceCostCr:directGroundReferenceCost/1e7,
      referenceLSTKCostINR:referenceLSTKCost,referenceLSTKCostCr:referenceLSTKCost/1e7,
      includeGroundReference,
      selectedLSTKCostINR:selectedLSTKCost,selectedLSTKCostCr:selectedLSTKCost/1e7,

      screeningSupplyBillingINRkg:rawRate?rawRate*supplyOverride:null,
      screeningNonSupplyLSTKINRkg:rawRate?rawRate*nonSupplyLSTKBillingFactor:null,
      screeningBaseLSTKINRkg:rawRate?rawRate*baseLSTKFactor:null,
      screeningReferenceLSTKINRkg:rawRate?rawRate*referenceLSTKFactor:null,
      supplyBillingShareOfBasePct,
      nonSupplyShareOfBasePct,
      directGroundReferenceSharePct,

      weightRange,supplyBillingRange,nonSupplyLSTKRange,baseLSTKRange,referenceLSTKRange,lstkRange,
      quantitySupplyBillingCostINR:supplyBillingCost*qty,
      quantityNonSupplyLSTKBillingCostINR:nonSupplyLSTKBillingCost*qty,
      quantitySelectedLSTKCostINR:selectedLSTKCost*qty,
      quantitySelectedLSTKCostCr:selectedLSTKCost*qty/1e7,

      material,fab,engineering,civil,ei,other,contingency,escalation,bottomUpTotal,bottomUpTotalCr:bottomUpTotal/1e7,
      designVolumeM3:designVol,
      supplyBillingCostPerM3:designVol>0?supplyBillingCost/designVol:null,
      lstkCostPerM3:designVol>0?selectedLSTKCost/designVol:null,
      poScopeClass:(COST_DATA&&COST_DATA.AUDIT&&COST_DATA.AUDIT.poScopeClass)||"Complete LSTK Installed Tank Package",
      foundationCivilIncluded:(COST_DATA&&COST_DATA.AUDIT)?!!COST_DATA.AUDIT.foundationCivilIncluded:true,
      governance,

      // Backward-compatible result aliases for existing integrations and saved exports.
      materialIdiotIndex:supplyOverride,
      serviceInstallationIdiotIndex:nonSupplyLSTKBillingFactor,
      baseCompleteIdiotIndex:baseLSTKFactor,
      installedReferenceIdiotIndex:referenceLSTKFactor,
      idiotIndex:supplyOverride,
      materialSupplyCostINR:supplyBillingCost,materialSupplyCostCr:supplyBillingCost/1e7,
      serviceInstallationCostINR:nonSupplyLSTKBillingCost,serviceInstallationCostCr:nonSupplyLSTKBillingCost/1e7,
      baseCompleteCostINR:baseLSTKCost,baseCompleteCostCr:baseLSTKCost/1e7,
      installedReferenceCostINR:referenceLSTKCost,installedReferenceCostCr:referenceLSTKCost/1e7,
      selectedCompleteCostINR:selectedLSTKCost,selectedCompleteCostCr:selectedLSTKCost/1e7,
      supplyCostINR:supplyBillingCost,supplyCostCr:supplyBillingCost/1e7,
      screeningFinishedSupplyINRkg:rawRate?rawRate*supplyOverride:null,
      screeningServiceINRkg:rawRate?rawRate*nonSupplyLSTKBillingFactor:null,
      screeningBaseCompleteINRkg:rawRate?rawRate*baseLSTKFactor:null,
      materialSharePct:supplyBillingShareOfBasePct,
      serviceSharePct:nonSupplyShareOfBasePct,
      materialRange:supplyBillingRange,
      serviceRange:nonSupplyLSTKRange,
      completeRange:lstkRange,
      quantityMaterialSupplyCostINR:supplyBillingCost*qty,
      quantityServiceInstallationCostINR:nonSupplyLSTKBillingCost*qty,
      quantitySelectedCompleteCostINR:selectedLSTKCost*qty,
      quantitySelectedCompleteCostCr:selectedLSTKCost*qty/1e7,
      supplyCostPerM3:designVol>0?supplyBillingCost/designVol:null,
      completeCostPerM3:designVol>0?selectedLSTKCost/designVol:null
    };
  }
  ...
  function overallStatus(gates,shell,roof,cal,eligibility){
    ...
    if(eligibility&&eligibility.available){
      ...
      else if(cal&&cal.domain&&!( ["IN_DOMAIN","SMALL_TANK_RULE"].includes(cal.domain.status) )) estimateStatus="REVIEW";
