(function(){
  ...
  function loadInput(o){
    if(!o) return;
    for(const id of scalarIds){ const el=$(id); if(el&&o[id]!=null) el.value=o[id]; }
    for(const id of checkIds){ const el=$(id); if(el&&o[id]!=null) el.checked=!!o[id]; }
    syncMaterialGroup($("shellMaterial"),$("shellGroup"));
    if(o.courses?.length) setCourses(o.courses); else generateCourses();
  }
  ...
  function renderCostEquationFlow(r){
    const c=r.cost;
    if(!c?.available){ $("costEquationFlow").innerHTML=`<h2><span class="equation-number">E</span>Cost calculation flow</h2><div class="equation-warning">Cost remains HOLD until an eligible weight estimate is available.</div>`; return; }
    $("costEquationFlow").innerHTML=`<h2><span class="equation-number">E</span>Cost calculation flow</h2><div class="equation-layout"><div class="formula-card"><span class="formula-label">Formula</span><div class="formula-display">C<sub>raw</sub> = W × 1,000 × R</div><div class="formula-display">C<sub>base</sub> = C<sub>raw</sub> × I<sub>base</sub></div><small>Material and service values are reconciled shares of the historical cost basis.</small></div><div class="formula-legend"><span><strong>W</strong> is eligible TankM weight in MT; 1,000 converts MT to kg; <strong>R</strong> is raw-steel rate.</span><span><strong>I<sub>base</sub></strong> is the selected base-complete cost index.</span><span>Low / base / high values are Class 3 estimate ranges, not a vendor quotation.</span></div></div>${worked("Current case",`C<sub>raw</sub> = ${eqNum(c.weightBasisMT,2)} × 1,000 × ₹${eqNum(c.rawSteelRate,2)} = <b>${fmtCr(c.rawSteelBasisCr)}</b>; C<sub>base</sub> = ${fmtCr(c.rawSteelBasisCr)} × ${eqNum(c.baseCompleteIdiotIndex,4)} = <b>${fmtCr(c.selectedCompleteCostCr)}</b>.`)}${equationTable(["Step","Live calculation"],[`<tr><td>Raw steel basis</td><td>${fmtMT(c.weightBasisMT)} × 1,000 × ₹${eqNum(c.rawSteelRate,2)}/kg = ${fmtCr(c.rawSteelBasisCr)}</td></tr>`,`<tr><td>Material supply</td><td>${fmtCr(c.rawSteelBasisCr)} × ${eqNum(c.materialIdiotIndex,4)} = ${fmtCr(c.materialSupplyCostCr)}</td></tr>`,`<tr><td>Service / installation</td><td>${fmtCr(c.rawSteelBasisCr)} × (${eqNum(c.baseCompleteIdiotIndex,4)} − ${eqNum(c.materialIdiotIndex,4)}) = ${fmtCr(c.serviceInstallationCostCr)}</td></tr>`,`<tr class="equation-total"><td>Base complete tank</td><td>${fmtCr(c.selectedCompleteCostCr)}</td></tr>`])}`;
  }
  ...
  function renderMto(r){
    ...
    defs("calibrationDetails",[["Model",esc(r.calibrated.mode||"-")],["Core MTO",fmtMT(r.calibrated.coreMtoMT)],["Global prediction",fmtMT(r.calibrated.globalPredictionMT)],["Local correction",fmtMT(r.calibrated.localAnchorCorrectionMT)],["Complete prediction",fmtMT(r.calibrated.predictedEmptyWeightMT)],["Completion uplift",`${fmtMT(r.calibrated.completionUpliftMT)}${completionUpliftPct===null?"":` (${fmt(completionUpliftPct,2)}% of Core MTO)`}`]]);
  ...
  function renderCost(r){
    const c=r.cost;
    if(!c?.available){
      $("rawSteelBasisCost").textContent=$("materialSupplyCost").textContent=$("serviceInstallationCost").textContent=$("baseCompleteCost").textContent="HOLD";
      defs("costDetails",[["Status","HOLD"],["Reason",esc(c?.governance||"No eligible weight estimate")]]); $("costRangeTableBody").innerHTML=""; renderCostEquationFlow(r); return;
    }
    $("rawSteelBasisCost").textContent=fmtCr(c.rawSteelBasisCr);
    $("materialSupplyCost").textContent=fmtCr(c.materialSupplyCostCr);
    $("serviceInstallationCost").textContent=fmtCr(c.serviceInstallationCostCr);
    $("baseCompleteCost").textContent=fmtCr(c.selectedCompleteCostCr);
    defs("costDetails",[["Weight basis",fmtMT(c.weightBasisMT)],["Raw steel rate",`₹${fmt(c.rawSteelRate,2)}/kg`],["Material Supply Index",fmt(c.materialIdiotIndex,2)],["Service / Installation Index",fmt(c.serviceInstallationIdiotIndex,2)],["Base Complete Index",fmt(c.baseCompleteIdiotIndex,2)],["Material share",`${fmt(c.materialSharePct,2)}%`],["Service share",`${fmt(c.serviceSharePct,2)}%`],["Quantity total",fmtCr(c.quantitySelectedCompleteCostCr)]]);
    notes("costNotes",[{text:c.governance},{text:"Service / Installation is the reconciled historical Engineering + Construction remainder. It is not API 650 design logic."}]);
    const rows=[["Material supply",c.materialRange,c.materialIdiotIndex],["Service / installation",c.serviceRange,c.serviceInstallationIdiotIndex],[c.includeGroundReference?"Installed/reference complete":"Base complete tank",c.completeRange,c.includeGroundReference?c.installedReferenceIdiotIndex:c.baseCompleteIdiotIndex]];
  ...
  function renderReport(r){
    ...
    $("reportBody").innerHTML=`
      <article class="owner-report">
        ...
        <section><h2>4. Owner Cost Intelligence</h2>${c?.available?`<table class="owner-table"><thead><tr><th>Cost Component</th><th>Amount</th><th>Share of Base Cost</th></tr></thead><tbody><tr><td>Material Supply</td><td>${fmtCr(c.materialSupplyCostCr)}</td><td>${fmt(c.materialSharePct,2)}%</td></tr><tr><td>Service / Installation</td><td>${fmtCr(c.serviceInstallationCostCr)}</td><td>${fmt(c.serviceSharePct,2)}%</td></tr><tr class="total-row"><td>Base Complete Tank</td><td>${fmtCr(c.selectedCompleteCostCr)}</td><td>100.00%</td></tr></tbody></table><p><strong>Quantity:</strong> ${c.tankQuantity} tank(s) &nbsp; <strong>Base cost per tank:</strong> ${fmtCr(c.selectedCompleteCostCr)} &nbsp; <strong>Total base owner cost:</strong> ${fmtCr(c.quantitySelectedCompleteCostCr)}</p>`:"<p>Cost is withheld because an eligible complete weight is unavailable.</p>"}</section>
  ...
  function saveCaseFile(){ const payload={format:"TankM Case",schemaVersion:1,savedAt:new Date().toISOString(),input:collectInput()}; download(caseFileName(),JSON.stringify(payload,null,2),"application/json"); $("overallStatusText").textContent="TankM case JSON file downloaded."; }
  async function loadCaseFile(file){
    if(!file) return;
    try{
      const payload=JSON.parse(await file.text());
      if(payload?.schemaVersion!=null&&payload.schemaVersion!==1) throw new Error(`Unsupported TankM case version: ${payload.schemaVersion}`);
  ...
  function exportCost(){ if(!lastResult?.cost?.available) return; const c=lastResult.cost; const rows=[["Layer","Factor","Cost INR","Cost Cr"],["Raw steel basis",1,c.rawSteelBasisINR,c.rawSteelBasisCr],["Material supply",c.materialIdiotIndex,c.materialSupplyCostINR,c.materialSupplyCostCr],["Service / installation",c.serviceInstallationIdiotIndex,c.serviceInstallationCostINR,c.serviceInstallationCostCr],["Base complete",c.baseCompleteIdiotIndex,c.baseCompleteCostINR,c.baseCompleteCostCr]]; download("tankm-cost.csv",rows.map(r=>r.map(csvCell).join(",")).join("\n"),"text/csv"); }
