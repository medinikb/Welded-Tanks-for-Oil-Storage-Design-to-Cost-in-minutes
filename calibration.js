(function(global){
  ...
  const MODEL_VERSION = "small-tank-class3";
  ...
  // A deterministic owner-side completion rule, validated against six small
  // tanks. It affects estimated complete empty weight only—never API 650 MTO.
  const SMALL_TANK_RULE = {
    factor: 1.50,
    coreMtoThresholdMT: 100,
    diameterThresholdM: 19,
    condition: "Core MTO < 100 MT OR diameter < 19 m",
    validation: {
      n: 6,
      mapePct: 9.830242790384835,
      minErrorPct: -8.164493480441324,
      maxErrorPct: 25.019230769230788,
      class3LowPct: -20,
      class3HighPct: 30,
      class3PassCount: 6
    },
    governance: "Owner-side Class 3 completion factor derived from six external small-tank actual-weight checks. It changes only the complete empty-weight estimate; API 650 Core MTO is unchanged."
  };
  ...
  function isSmallTank(input, coreMtoMT){
    const D=n(input.diameter), core=n(coreMtoMT);
    return core>0 && (core<SMALL_TANK_RULE.coreMtoThresholdMT || (D>0 && D<SMALL_TANK_RULE.diameterThresholdM));
  }

  function domainStatus(input, coreMtoMT){
    const D=n(input.diameter), core=n(coreMtoMT);
    if(isSmallTank(input,core)){
      const triggers=[];
      if(core<SMALL_TANK_RULE.coreMtoThresholdMT) triggers.push(`Core MTO ${core.toFixed(2)} MT < ${SMALL_TANK_RULE.coreMtoThresholdMT} MT`);
      if(D>0 && D<SMALL_TANK_RULE.diameterThresholdM) triggers.push(`diameter ${D.toFixed(2)} m < ${SMALL_TANK_RULE.diameterThresholdM} m`);
      const issues=[`Small-tank Class 3 rule active: ${SMALL_TANK_RULE.factor.toFixed(2)} × Core MTO (${triggers.join("; ")}).`];
      if(input.roofType==="efr" && input.floatingRoofSubtype==="double") issues.push("Double-deck EFR roof dead-load basis remains a screening input unless verified project data is entered.");
      return {status:"SMALL_TANK_RULE",issues,smallTankRuleApplied:true,triggers};
    }
    const issues=[];
  ...
  function predictCompleteWeight(input, coreMtoMT){
    ...
    if(isSmallTank(input,core)){
      const factor=SMALL_TANK_RULE.factor;
      const adjusted=core*factor;
      return {
        modelVersion: MODEL_VERSION,
        coreMtoMT: core,
        globalPredictionMT: globalPred,
        globalPredictionDiagnosticOnly: true,
        localAnchorWeight: 0,
        localAnchorCorrectionMT: 0,
        smallTankFactor: factor,
        smallTankFactorCorrectionMT: adjusted-core,
        predictedEmptyWeightMT: adjusted,
        completionUpliftMT: adjusted-core,
        completionUpliftPct: core>0?(adjusted/core-1)*100:null,
        mode: "SMALL_TANK_CORE_FACTOR",
        domain,
        anchor: IFR_SMALL_ANCHOR,
        anchorGlobalPredictionMT: ANCHOR_GLOBAL_PREDICTION,
        anchorResidualMT: ANCHOR_RESIDUAL_MT,
        governance: SMALL_TANK_RULE.governance
      };
    }

    // Medium and large tank completion logic is intentionally unchanged.
    ...
    return {
      ...
      coreMtoMT: core,
      globalPredictionMT: globalPred,
      globalPredictionDiagnosticOnly: false,
      localAnchorWeight: w,
      localAnchorCorrectionMT: w*ANCHOR_RESIDUAL_MT,
      smallTankFactor: null,
      smallTankFactorCorrectionMT: 0,
      predictedEmptyWeightMT: adjusted,
      completionUpliftMT: adjusted-core,
      completionUpliftPct: core>0?(adjusted/core-1)*100:null,
      mode,
  ...
  const api={
    MODEL_VERSION, GLOBAL_MODEL, SMALL_TANK_RULE, IFR_SMALL_ANCHOR,
    ANCHOR_GLOBAL_PREDICTION, ANCHOR_RESIDUAL_MT,
    featureVector, globalPredictionFromFeatures, globalPrediction,
    localAnchorWeight, isSmallTank, domainStatus, predictCompleteWeight
  };
